...
# have been removed
#####################################################
ex_seq <- readRDS("data/pre_processed/excel_sequences_SPS_format.RData")
print(head(ex_seq, n=20), format = "SPS")

# check the length of the sequences
ex_seq_length <- as.vector(seqlength(ex_seq))
summary(ex_seq_length)
hist(ex_seq_length, main = NULL, xlab = "Sequence length")
quantile(x = ex_seq_length, probs = seq(0.9, 1, 0.01))

# Examine state distribution by time points
seqdplot(ex_seq, main = "Distribution of communication intents over the Excel course", 
         cpal=col_pallet, with.legend=FALSE) #axes=FALSE, border=NA)
seqlegend(ex_seq, cex = 0.75, cpal = col_pallet)

# Sequence frequency plot of the 20 most frequent sequences with bar width proportional
# to the frequencies
seqfplot(ex_seq, main = "Excel course: 20 most freq. sequences", 
         with.legend=FALSE, axes=F, idxs=1:20, cpal = col_pallet)
seqlegend(ex_seq, cpal = col_pallet)

# Sequence frequency plot of the 20 most frequent sequences with above median length
seq_med_len <- median(ex_seq_length)
seqfplot(ex_seq[ex_seq_length > seq_med_len,], idxs = 1:20,
         main = "Excel course: 20 most freq. seq. w/ above median length", 
...
ex_seq_freq <- seqtab(ex_seq, idxs = 0) %>%
  attr(which = "freq") 
ex_seq_freq$seq <- unlist(attr(ex_seq_freq$Freq, "dimnames"))
ex_seq_freq <- ex_seq_freq %>% 
  arrange(desc(Freq)) %>%
  select(seq, Freq, Percent)
head(ex_seq_freq, n = 20)
tail(fp_seq_freq, n = 20)
# the frequent ones are rather homogenious - all top20 sequences have at most
# two different thread types

# Plot the mean number of communication intent types per sequence
seqmtplot(ex_seq, main="Mean number of communication intent types per sequence", 
...
ex_ent <- seqient(ex_seq)
# compare entropy values to sequences
ex_first_20 <- as.character(print(head(ex_seq, n=20), format = "SPS"))
cbind(Seq = ex_first_20, Entropy = round(ex_ent[1:20,1], digits = 3))
# plot the entropy
hist(ex_ent, xlab = "Entropy", main = NULL)

# Check the transition rates
round(seqtrate(ex_seq), digits = 2)
...
dist_euclid <- seqdist(seqdata = ex_seq, 
                        method = "EUCLID", 
                        norm = "auto",
                       step = 1)
ex_euclid_1_ward <- agnes(dist_euclid, diss = T, method = "ward")
plot(ex_euclid_1_ward)

# examine solutions with 3, 4, and 5 clusters
ward_cl3 <- cutree(ex_euclid_1_ward, k = 3)
table(ward_cl3)
ward_cl4 <- cutree(ex_euclid_1_ward, k = 4)
table(ward_cl4)
ward_cl5 <- cutree(ex_euclid_1_ward, k = 5)
table(ward_cl5)

## plot the state distribution at each time point for each cluster
seqdplot(seqdata = ex_seq, 
         group = factor(ward_cl4), 
         main = "Distribution of thread types over the course",
         with.legend=FALSE, cpal = col_pallet)
seqlegend(ex_seq, cpal = col_pallet, cex = 0.75)

# plot most frequent sequences from each cluster
seqfplot(ex_seq, group = factor(ward_cl4),
...
cl <- 4
seqfplot(ex_seq[ward_cl4 == cl,],
         main = paste("Freq. plot for cluster",cl), 
         with.legend=FALSE, axes=F, idxs=1:20, 
         cpal = col_pallet)
seqtab(ex_seq[ward_cl5 == cl,], idxs = 1:20)
hist(seqient(ex_seq[ward_cl5 == cl,]), xlab = "Entropy", main = NULL)
# 4 clusters seems to be the best solution
...
ward_cl4_var <- vector()
for(clust in 1:4) {
  clust_dist <- seqdist(seqdata = ex_seq[ward_cl4 == clust,],
                        method = "EUCLID", step = 1, norm = "auto")
  ward_cl4_var[clust] <- dissvar(clust_dist)
}
round(ward_cl4_var, digits = 2)
# 1.22 0.65 0.97 2.03
# The results suggest high variability among sequences in the 4th cluster;
# not much dispersion in the 2nd and 3rd clusters.
# This is expected as the 4th cluster is with longest sequences, whereas the 
# the 2nd and the 3rd are with much shorter sequences
...
for(clust in 1:4) {
  clust_dist <- seqdist(seqdata = ex_seq[ward_cl4 == clust,],
                        method = "EUCLID", step = 1, norm = "auto")
  seqrplot(seqdata = ex_seq[ward_cl4 == clust,], 
           diss=clust_dist, 
           criterion="density", pradius=0.2, coverage=0.5,
...
ex_seq_df <- readRDS("data/pre_processed/excel_sequences_df.RData")
ex_ward_cl4 <- data.frame(student_id = ex_seq_df$student_id,
                          cl4 = ward_cl4)
saveRDS(ex_ward_cl4, "clustering/excel_ward_euclid_4_clust.RData")
...
# of sequences 
dist_euclid_2 <- seqdist(seqdata = ex_seq,
                     method = "EUCLID",
                     norm = "auto",
                     step = median(ex_seq_length))
ex_euclid_2_ward <- agnes(dist_euclid_2, diss = TRUE, method = "ward")
plot(ex_euclid_2_ward)

# examine solutions with 3 and 4 clusters
ward_2_cl4 <- cutree(ex_euclid_2_ward, k = 4)
table(ward_2_cl4)
ward_2_cl3 <- cutree(ex_euclid_2_ward, k = 3)
table(ward_2_cl3)
## plot the state distribution at each time point for each cluster
seqdplot(seqdata = ex_seq, 
         group = factor(ward_2_cl4), 
         main = "Distribution of thread types over the course",
         with.legend=FALSE, cpal = col_pallet)
seqlegend(ex_seq, cpal = col_pallet, cex = 0.75)

# plot most frequent sequences from each cluster
seqfplot(ex_seq, group = factor(ward_2_cl4),
         main = "Sequence frequency plot", 
         with.legend=FALSE, axes=F, idxs=1:20, 
         cpal = col_pallet)
# Not that bad...
...
# associate the clusters with student ids and save them
# for later processing
ex_ward_2_cl4 <- data.frame(student_id = ex_seq_df$student_id,
                            cl4 = ward_2_cl4)
saveRDS(ex_ward_2_cl4, "clustering/excel_ward_2_euclid_4_clust.RData")
...
subs_cost <- features_based_subs_cost(ex_seq)
dist_om <- seqdist(seqdata = ex_seq, 
                   method = "OM", sm = subs_cost$sm, indel = 0.75,
                   norm = "auto")
ex_OM_ward <- agnes(dist_om, diss = TRUE, method = "ward")
plot(ex_OM_ward)
# examine solutions with 4 and 6 clusters
ward_3_cl4 <- cutree(ex_OM_ward, k = 4)
table(ward_3_cl4)
ward_3_cl6 <- cutree(ex_OM_ward, k = 6)
table(ward_3_cl6)
seqdplot(seqdata = ex_seq, 
...
seqfplot(ex_seq, group = factor(ward_3_cl4),
         main = "Seq. freq. plot", 
         with.legend=FALSE, axes=F, idxs=1:20, 
         cpal = col_pallet)
...
# Compute the level of discrepancy for each seq. cluster
ward_3_cl4_var <- vector()
for(clust in 1:4) {
  clust_dist <- seqdist(seqdata = ex_seq[ward_3_cl4 == clust,],
                     method = "OM", sm = subs_cost$sm, indel = 0.75)
  ward_3_cl4_var[clust] <- dissvar(clust_dist)
}
round(ward_3_cl4_var, digits = 2)
# 2.78 0.52 0.54 0.46
# The results suggest high variability among sequences in the 1st cluster;
# the other 4 clusters are fully fine, that is, not much dispersion is present.
...
for(clust in 1:4) {
  clust_dist <- seqdist(seqdata = ex_seq[ward_3_cl4 == clust,],
                        method = "OM", sm = subs_cost$sm, indel = 0.75)
  seqrplot(seqdata = ex_seq[ward_3_cl4 == clust,], 
           diss=clust_dist, 
...
# Associate the clusters with student ids and save them
# for later processing
ex_ward_om_cl4 <- data.frame(student_id = ex_seq_df$student_id,
                           cl4 = ward_3_cl4)
saveRDS(ex_ward_om_cl4, "clustering/excel_ward_OM_4_clust.RData")
...
forum_features <- read.csv("data/lca_features/lca_groups_excel.csv")
str(forum_features)
...
euclid_clusters <- readRDS("clustering/excel_ward_euclid_4_clust.RData") 
...
euclid_clust_stats <- summary.stats(euclid_clust_data[,c(3,4)], euclid_clust_data$cl4)
kable(euclid_clust_stats, format = 'rst')
# check the distribution of core posters
with(euclid_clust_data, table(core_poster, cl4))
...
kruskal.test(engaged_count ~ cl4, data = euclid_clust_data)
# Kruskal-Wallis chi-squared = 106.72, df = 3, p-value < 2.2e-16
kruskal.test(engaged_scope ~ cl4, data = euclid_clust_data)
# Kruskal-Wallis chi-squared = 65.192, df = 3, p-value = 4.563e-14
# =>
...
engage_cnt_comparison <- pairwise.compare.Mann.Whitney(euclid_clust_data, 'engaged_count', 'cl4', 4) 
kable(x = engage_cnt_comparison, format = 'rst')

# Now, do pairwise comparisons for the engaged_scope feature
engage_scope_comparison <- pairwise.compare.Mann.Whitney(euclid_clust_data, 'engaged_scope', 'cl4', 4) 
...
# Comparison of the clusters obtained by applying the Ward's algorithm 
# on sequence similarities computed using the metric sensitive to 
# timing of states (Euclid distance with step=median(seq_length))
#######################################################################

# Read in cluster assignments
euclid_clusters <- readRDS("clustering/excel_ward_2_euclid_4_clust.RData") 
# Merge the cluster assignments w/ forum features data, keeping only
# the data for students with hand-coded threads
euclid_clust_data <- merge(x = euclid_clusters, y = forum_features,
                           by = "student_id", all.x = TRUE, all.y = FALSE)
...
# check the distribution of core posters
with(euclid_clust_data, table(core_poster, cl4))
with(euclid_clust_data, round(prop.table(table(core_poster, cl4), margin = 2), digits = 2))

# Use statistical tests to compare clusters based on the given features
kruskal.test(engaged_count ~ cl4, data = euclid_clust_data)
# Kruskal-Wallis chi-squared = 76.145, df = 3, p-value < 2.2e-16
kruskal.test(engaged_scope ~ cl4, data = euclid_clust_data)
# Kruskal-Wallis chi-squared = 38.563, df = 3, p-value = 2.148e-08
# =>
# Singificant difference is present among the clusters for both examined features
# Use pairwise tests to examine where exactly (i.e. between which cluster pairs) 
# the difference is present
...
# Read in cluster assignments
om_clusters <- readRDS("clustering/excel_ward_OM_4_clust.RData") 
# Merge the cluster assignments w/ forum features data, keeping only
# the data for students with hand-coded threads
...
with(om_clust_data, round(prop.table(table(core_poster, cl4), margin = 2), digits = 2))
...
# Kruskal-Wallis chi-squared = 114.22, df = 3, p-value < 2.2e-16
# =>
# Singificant difference is present among the clusters for both examined features
# Use pairwise tests to examine where exactly (i.e. between which cluster pairs) 
# the difference is present
